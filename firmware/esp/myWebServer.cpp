...
extern int ST1; //Set_Extruder_Temp
extern int CT1; //Current_Extruder_Temp
extern float O1; // OutputValueExtruder;
extern int ST2; //Set_Bed_Temp
extern int CT2; //Current_Bed_Temp
extern float O2; // OutputValueBed;
...
void handleIndex() {
  ...
  server.send_P(200, "text/html", PAGE_INDEX, sizeof(PAGE_INDEX));
}
...
void handlePIDConfig() {
  //send GZ version of embedded config
  server.sendHeader("Content-Encoding", "gzip");
  server.send_P(200, "text/html", PAGE_PIDSETUP, sizeof(PAGE_PIDSETUP));
}

void sendPIDUpdate(String fname) {
  // pid.json file saved so we have to send this to atmel
...
void handleDashBoard() { // we willsend dash board file
  //send GZ version of embedded config
  server.sendHeader("Content-Encoding", "gzip");
  server.send_P(200, "text/html", PAGE_DASHBOARD, sizeof(PAGE_DASHBOARD));
}

void handleAbout() { // we willsend dash board file
  //send GZ version of embedded config
  server.sendHeader("Content-Encoding", "gzip");
  server.send_P(200, "text/html", PAGE_ABOUT, sizeof(PAGE_ABOUT));
}

void handleDashJsonUpdate() {
  //dashboard data will be send to webpage from this function
  DebugPrintln("handleJsonUpdate");


  String InJson = "{\"ST1\":"+String(ST1)+
  ",\"ST2\":"+String(ST2)+
  ",\"CT1\":"+String(CT1)+
  ",\"CT2\":"+String(CT2)+
  ",\"O1\":"+String(O1)+
...
//  server.streamFile(InJson, "application/json");
  server.send ( 200, "application/json",InJson);
}
...
void handleJsonSave()
{
    ...
    file.close();
    //delay(100);
    //if (fname =="/pid.json") {sendPIDUpdate(fname);}; //nuri
	}
...
void handleRoot() {  //handles root of website (used in case of virgin systems.)
	...
	if (!handleFileRead("/")) {   //if new system without index we either show wifisetup or if already setup/connected we show filebrowser for config.
		if (isAdmin()) {
			...
			else { handleIndex();}//HandleFileBrowser(); }
		}
	}
...
//SERVER INIT
    ...
    delay(2000);// let people to see the IP address.
...
//list directory
  ...
  server.on("/wifisetup.html", handleWifiConfig); //nuri
  server.on("/pidsetup.html", handlePIDConfig); //nuri
  server.on("/dashboard.html", handleDashBoard); //nuri
  server.on("/about.html", handleAbout); //nuri
  server.on("/jsonUpdate", handleDashJsonUpdate); //Nuri
